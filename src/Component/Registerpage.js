...
//                   <MenuItem key={index + 1990} value={(index + 1990).toString()}>{index + 1990}</MenuItem>
//                 ))}
//               </Select>
//             </Box>
...
const Registerpage = () => {
  const [formValue, setFormValue] = useState({
    ...
    dobDay: '',
    dobMonth: '',
    dobYear: '', // Initialize dobYear to an empty string
    password: ''
  });
  ...
  const handleSubmit = () => {
    // Concatenate dobDay, dobMonth, and dobYear into user_dob
    const user_dob = `${formValue.dobDay} ${formValue.dobMonth} ${formValue.dobYear}`;
    
    // Create a new object with updated user_dob field
    const updatedFormValue = {
      ...formValue,
      user_dob: user_dob
    };

    // Update the formValue state with the new object
    setFormValue(updatedFormValue);

    // Now you can submit the formValue or perform any other actions
